...
def main():
    ...
    print(f"    (direct FdmLocalSource call, physical FDM column names)")
    ...
    print(f"    -> to get sector/country you ALSO need party_demographic() and")
    print(f"       party_locator() and a manual merge -- see fdm_worklist.py:147-157")

    print(f"\n--- FOUNDATION BUILT, NOT YET WIRED IN: the same client through CanonicalSource ---")
    print(f"    (schema-agnostic call, canonical field names, joins already applied)")
    party = canonical.read("Party", as_at=as_of, party_id=prty_id).iloc[0]
    print(f"    canonical.read('Party', as_at=as_of, party_id=prty_id) ->")
    ...
    print(f"    the legacy schema, or a schema that doesn't exist yet -- IF a binding")
    print(f"    exists for it. Only config/bindings/fdm.yaml exists today.")
    ...
    if not deposit.empty:
        ...
        print(f"          'AGRMNT_LDGR_BAL_AMT' by name (see its REQUIRED_COLUMNS).")
        print(f"  After the rewire, it would require 'balance' instead, and read via")
        print(f"          canonical.read('BalanceObservation', account_id=...) --")
        ...
        print(f"  cash_buildup.py stop caring whether 'balance' came from FDM's")
        print(f"  AGRMNT_LDGR_BAL_AMT, the legacy schema's own balance column, or a")
        print(f"  third schema nobody has written yet -- as long as ITS binding maps")
        print(f"  'balance' to whatever that schema calls it.")
